...
def main():
    ...
    # 환경 파라미터
    env_kwargs = dict()               # 학습용
    eval_env_kwargs = dict()          # 평가용(고정 목표 등)
    # 선택적으로 config에서 평가용 고정 목표를 받아 사용합니다.
    # 예시: sb3_sac.yaml에
    # eval:
    #   fixed_targets: [24.0, 25.0, 26.0, 25.0]
    eval_fixed_targets = (cfg.get("eval", {}) or {}).get("fixed_targets")
    if eval_fixed_targets is not None:
        eval_env_kwargs["fixed_targets"] = eval_fixed_targets

    # === 벡터 환경 / 평가 환경 ===
    train_env = build_vec_env(cfg, env_kwargs, for_eval=False)
    eval_env  = build_vec_env(
        cfg, eval_env_kwargs, for_eval=True,
        obs_rms_source=train_env if isinstance(train_env, VecNormalize) else None
    )
